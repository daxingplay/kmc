...
Compiler.prototype._getModuleRealPathByParent = function (requiredModule, parentModule) {
    ...
    if (requiredModule.match(/^\./) && _.isObject(parentModule)) {
        return {
            name: utils.normalizePath(parentModule.name, utils.addIndexSuffix(requiredModule)),
            path: path.resolve(path.dirname(parentModule.path), utils.addPathSuffix(requiredModule))
        }
    } else if (!requiredModule.match(/\//) || requiredModule.match(/^gallery\//)) {
        // may be kissy module.
        return {name: requiredModule, path: requiredModule};
    } else {
        for (var i = 0; i < self.packages.length; i++) {
            ...
            if (mod && mod[1]) {
                ...
                if (fs.existsSync(modulePath)) {
                    return {name: requiredModule, path: modulePath};
                }
            }
        }
    }
    var lastPath = path.resolve(path.dirname(parentModule.path), utils.addPathSuffix(requiredModule));
    return {name:requiredModule,path:fs.existsSync(lastPath) ? lastPath : requiredModule};
};
                ...
                if (_.indexOf(self.fileList, modRealPath) === -1) {
                    ...
                    if (modType === 'js') {
                        ...
                        _.forEach(modRequires, function (subModPath) {
                            var subMod = self._getModuleRealPathByParent(subModPath, mod);
                            var name = subMod.name;
                            subMod = self.analyze(subMod.path);
                            if (subMod) {
                                subMod.name = name;
