...
Compiler.prototype._alias = function(modName){
    ...
    if(self.config.modules && self.config.modules[modName]){
        ...
        if(modConfig.alias){
            ...
            if(modNew.pkg && self.packages[modNew.pkg]){
                var modPkg = self.packages[modNew.pkg];
                if(modPkg.ignorePackageNameInUri){
                    modNew.path = path.resolve(modPkg.path, '../', modNew.name);
                }else{
                    modNew.path = path.resolve(modPkg.path, modNew.name);
                }
