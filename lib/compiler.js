...
//Compiler.prototype._kissyPackage = function(modRequired){
//    if(/[^\/]+/.test(modRequired)){
//        return {
//            name: 'kissy'
//        };
//    }
//    if(/^gallery\//.test(modRequired)){
//        return {
//            name: 'gallery'
//        };
//    }
...
Compiler.prototype._buildCombo = function(mod){
    ...
    _.forEach(mod.dependencies, function(subMod){
        if((subMod.pkg == 'kissy' || subMod.status != 'ok') && !self.buildAnalyzedModules.indexOf(subMod.name)){
            self.buildComboModules.push(subMod.name);
            !_.isEmpty(subMod.dependencies) && self._buildCombo(subMod);
        }
    });
    return self.buildComboModules;
};
        ...
        default :
            ...
            if(_.indexOf(self.fileList, modRealPath) === -1){
                ...
                mod = {
                    ...
                    dependencies: [],
                    dependencyTree: {},
                    charset: utils.convertCharset(modPkg.charset)
                };
...
Compiler.prototype.build = function(inputFilePath, outputFilePath, outputCharset){
    ...
    _.forEach(self.analyzedModules, function(modName){
        ...
        modContent = self._addModuleName(modContent, mod.name) + '\r\n';
    ...
    !self.config.silent && console.info('[ok]'.bold.green+' ===> %s', inputFilePath, outputFilePath);
...
Compiler.prototype.combo = function(){
    ...
    if(_.isObject(self.modules)){
        _.forEach(self.modules, function(mod, modName){
            if(!self.combinedModules.indexOf(modName) && !_.isEmpty(mod.dependencies)){
                var requires = [];
                _.forEach(mod.dependencies, function(subMod){
    ...
    return content.length ? "KISSY.config('modules', {\n " + content.join(",\n") + " \n});" : "";
};
